...
if(isset($_SESSION['statut']) && !empty($_SESSION['statut']) && $_SESSION['statut'] == "admin" && isset($_SESSION['nom']) && !empty($_SESSION['nom'])):
    ...
    if(isset($_POST['nom']) && !empty($_POST['nom']) && isset($_POST['user']) && !empty($_POST['user']) && isset($_POST['password']) && !empty($_POST['password']) && isset($_POST['inlineRadioOptions']) && !empty($_POST['inlineRadioOptions']) && isset($_POST['radiogpe']) && !empty($_POST['radiogpe'])):
        ...
        if($result > 0){
            ...
            $createuser = $pdo->prepare("INSERT INTO `utilisateurs` (`nom` , `identifiant`, `mdphash`,`statut`) VALUES (:nom, :user, :passworded, :statut)");
            $createuser->bindParam(':nom',$nom,PDO::PARAM_STR);
            $createuser->bindParam(':user',$user,PDO::PARAM_STR);
            $createuser->bindParam(':passworded',$password,PDO::PARAM_STR);
            $createuser->bindParam(':statut',$statut,PDO::PARAM_STR);
            $createuser->execute();

            $belonging = $pdo ->prepare("INSERT INTO `appartenance` (`utilisateurs_nom` , `catRS_nom`) VALUES (:nom, :groupe)");
            $belonging->bindParam(':nom',$nom,PDO::PARAM_STR);
            $belonging->bindParam(':groupe',$groupe,PDO::PARAM_STR);
            $belonging ->execute();
    ...
    <div class="col s12 m6">
      <div class="cardacc">
        <div class="card-content">
            ...
            <?php } else {?><p id="dct" style="text-align:center;"><?=$error?></p>
            <br>
            <div class="card-action">
                <button id="suppr" type="submit"class="waves-effect waves-light btn hoverable" onclick="self.location.href='admincontent.php'">Réessayer</button><?php }?>
            </div>
        </div>
    </div>
</div>
